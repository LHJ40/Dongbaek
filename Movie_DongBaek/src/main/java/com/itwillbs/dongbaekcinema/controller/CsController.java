...
//		model.addAttribute("board_csNotice", board_csNotice);
	...
	public List<CsVO> fag_data(@RequestParam("cs_type") String cs_type, @RequestParam("pageNum") int pageNum) {
		
		
		// 받아온 값으로(cs_type) 레코드 조회
		// CsService - getFaq()
		// 파라미터 : String(cs_type, pageNum)		리턴타입 : CsVO(faq)
		List<CsVO> faq = service.getCsFaq(cs_type, pageNum);
